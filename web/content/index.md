...
screenshot_alt: "@sebastienrousseau/stylelint-config architecture and rule enforcement."
footer_note: "@sebastienrousseau/stylelint-config is part of the @sebastienrousseau/config suite. Published under Apache-2.0 OR MIT."
copyright: "© 2026 Sebastien Rousseau. Licensed under Apache-2.0 OR MIT."
translation_key: "home"
title: "@sebastienrousseau/stylelint-config — Shareable Stylelint configuration rules for modern CSS, SCSS, and PostCSS styling quality."
description: "Shareable Stylelint configuration rules for modern CSS, SCSS, and PostCSS styling quality."
keywords: "stylelint-config, stylelint, css linter, scss, postcss, styling rules, developer tools, sebastien rousseau, standard"
hero_tag: "v0.0.4"
hero_tagline: "Strict CSS, SCSS, and PostCSS styling quality standards"
headline: "@sebastienrousseau/stylelint-config"
lead: "Shareable Stylelint configuration rules for modern CSS, SCSS, and PostCSS styling quality."
cta_primary: "Install @sebastienrousseau/stylelint-config"
...
snippet_caption: "Install @sebastienrousseau/stylelint-config"
snippet_code: "npm install --save-dev @sebastienrousseau/stylelint-config"
...
## What is @sebastienrousseau/stylelint-config?

**@sebastienrousseau/stylelint-config** is an enterprise-grade, shareable configuration for [Stylelint](https://github.com/sebastienrousseau/stylelint-config)—the mighty, modern linter that helps you avoid errors and enforce conventions in your styles. Designed to deliver vetted, deterministic defaults, this package eliminates manual setup boilerplate and establishes rigorous code quality standards across modern software development pipelines.

By adopting this configuration, engineering teams ensure unified consistency across local development, continuous integration, and automated deployments. The package adheres strictly to the OpenSSF Best Practices badge program (ID: 14520) and achieves top-tier security ratings via Scorecard audits.
...
## Why Developers Choose @sebastienrousseau/stylelint-config

Modern JavaScript and TypeScript repositories frequently suffer from configuration drift, fragmented standards, and maintenance overhead. **@sebastienrousseau/stylelint-config** directly solves these challenges:
...
Integrating `@sebastienrousseau/stylelint-config` into any new or existing repository requires only three simple steps.
...
### Step 2: Configure `.stylelintrc.json`

Create or update your `.stylelintrc.json` file at the root of your project to extend the shared configuration:

```json
{
  "extends": "@sebastienrousseau/stylelint-config"
}
```

Learn how to customize specific rules and override settings in the [Usage Guide](usage/).

### Step 3: Run Stylelint in Your Workflow

Execute Stylelint directly or configure standard quality scripts in your `package.json`:

```bash
npx stylelint "**/*.css"
```

Lint stylesheets for invalid properties, selector specificity issues, and formatting violations.
...
### Does this configuration support modern CSS features?
Yes. It fully supports CSS nesting, modern color functions (oklch, lab), container queries, CSS variables, and vendor prefix best practices.

### Can I override individual rules in my project?
Yes. Every configuration allows local overrides directly within your `.stylelintrc.json` file or inline settings. Detailed override recipes are documented in the [Usage Guide](usage/).

### Is @sebastienrousseau/stylelint-config safe for production CI/CD?
