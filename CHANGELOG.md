...
- Removed the `indentation`, `max-empty-lines` and `string-quotes` rules. Stylelint
  deleted them in 16, which this package requires, so every consumer saw three
  "Unknown rule" errors on every file. Formatting is Prettier's job.
